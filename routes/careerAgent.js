...
router.post('/', verifyToken, validateCareerAgentRelationship, async (req, res) => {
  try {
    ...
    const existingActiveRelationship = await Connection.findOne({ 
    ...
    const existingPendingRelationship = await Connection.findOne({ 
    ...
    if (existingPendingRelationship) {
      return res.status(400).json({
        ...
        existingConnectionId: existingPendingRelationship.connectionId
      });
    }

    // Create new career agent relationship
    const careerAgentRelationship = new Connection({
      careerAgentId,
      candidateId,
      connectionType: 'careerAgent',
      message: message || '',
  ...
  try {
    ...
    if (existingPendingRequest) {
      return res.status(400).json({
        ...
        existingConnectionId: existingPendingRequest.connectionId
      });
    }

    // Create new career agent request
    const careerAgentRequest = new Connection({
      careerAgentId,
      candidateId,
      connectionType: 'careerAgent',
...
router.post('/propose', verifyToken, async (req, res) => {
  try {
    ...
    const existingRelationship = await Connection.findOne({ 
      careerAgentId: careerAgentId,
      candidateId: candidateId,
      relationshipStatus: { $in: ['pending', 'proposed'] }
    });
...
router.get('/proposals/received', verifyToken, async (req, res) => {
  try {
    ...
    const proposalsReceived = await Connection.find({
      candidateId: candidateId,
      relationshipStatus: 'proposed'
    })
    .populate('careerAgent', 'firstName lastName headline industry location profilePictureUrl')
...
router.get('/proposals/sent', verifyToken, async (req, res) => {
  try {
    ...
    const proposalsSent = await Connection.find({
      careerAgentId: careerAgentId,
      relationshipStatus: 'proposed'
    })
    .populate('candidate', 'firstName lastName headline industry location profilePictureUrl')
...
router.get('/connected', verifyToken, async (req, res) => {
  try {
    ...
    const connectedRelationships = await Connection.find(query)
      .populate('candidate', 'firstName lastName headline industry location profilePictureUrl')
      .populate('careerAgent', 'firstName lastName headline industry location profilePictureUrl')
      .limit(parseInt(limit))
      .sort({ createdAt: -1 });
...
router.get('/agent/:careerAgentId', verifyToken, async (req, res) => {
  try {
    ...
    const relationships = await Connection.find(query)
      .populate('candidate', 'firstName lastName headline industry location profilePictureUrl')
...
router.put('/:relationshipId', verifyToken, async (req, res) => {
  try {
    ...
    const relationship = await Connection.findOneAndUpdate(
      { _id: relationshipId },
      updateData,
...
router.delete('/:relationshipId', verifyToken, async (req, res) => {
  try {
    ...
    const relationship = await Connection.findOneAndDelete({ _id: relationshipId });
